...
mod1_int = lmer(yield_mgha ~ 1 + (1|rep), 
                data = reg_dfw
                ) # adding 1 means yield is not a function of anything i.e overall mean
...
```{r nd}
nd = data.frame(sr_ksha = seq(40, 120, 1)) # lower, upper, step
...
nd = nd %>%
  mutate(mod1_yield_mgha = predict(mod1_int,
                                   nd,
                                   re.form = NA
                                   )) #re.form means do not include random effect in the prediction
...
# our type of model is of Lmer so we use 
mod1_int_boots = bootMer(
  mod1_int,
  predict.fun,
  nsim = 200
) %>% 
  confint() %>% 
  as.data.frame() %>%
  rename(mod1_int_lcl = `2.5 %`, mod1_int_ucl = `97.5 %`)
...
ggplot(reg_dfw,
  ...
  geom_line(data =  nd,
            ...
            color = "forestgreen") + 
  geom_ribbon(data = nd,
              aes(y = mod1_yield_mgha, ymin = mod1_int_lcl, ymax = mod1_int_ucl),
              fill = "grey",
              alpha = 0.5) 
```
...
mod2_lin = lmer(yield_mgha ~ sr_ksha + (1|rep),
                data = reg_dfw) # sr_ksha - exploratory varaible
...
# Model fitting
mod3_quad  = lmer(yield_mgha ~ sr_ksha + I(sr_ksha^2) + (1|rep),
                  data = reg_dfw
                  ) # I is known as Identity
...
# a = intercept of the linear part
# b = slope of the linear part
# xs = breakpoint (x-value where the plateau starts)
# sr_ksha = your predictor variable (numeric vector)
```
